...
async function carregarRelatorio() {
  ...
  d.perguntas.forEach((p, i) => { if (p.tipo !== 'nota') return; area.insertAdjacentHTML('beforeend', `<section class="card report-question"><h2>${p.ordem}. ${p.pergunta}</h2><p class="escala">📏 Escala: ${p.escala || '0 a 10'}</p><p class="muted">${p.respostas} resposta(s) · Média: ${p.media ?? '—'}</p><div class="charts"><div class="chart-wrap"><canvas id="pergunta-${i}"></canvas></div><div class="chart-wrap"><canvas id="media-${i}"></canvas></div></div></section>`); criarGraficoNotas(document.querySelector(`#pergunta-${i}`),p.notas); criarGraficoMedia(document.querySelector(`#media-${i}`),p.media); });
}
carregarRelatorio();
